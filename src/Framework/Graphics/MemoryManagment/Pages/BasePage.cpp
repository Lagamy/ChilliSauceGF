	...
	PoolId BasePage::addBuffer(VkDeviceSize size_, BufferTypeEnum bufferType_, const char* uploadName_)
	{
		...
		// for(FreeSpace& rFreeSpace : this->freeSpacePerBlock)
		{
			...
			for(uint32_t i = 0; i < rFreeSpace.aliveIntervals.size(); i++)
			{
                ...
                uint64_t alignedMemoryIntervalStart = alignUp(rMemoryInterval.start, this->memReqs[bufferType_].alignment);            
				
                VkDeviceSize intervalSize = rMemoryInterval.end - alignedMemoryIntervalStart + 1;

				if(size_ <= intervalSize) // suitable for this buffer 
				{
					rFreeSpace.intervalByFirst.erase(rMemoryInterval.start); 
					rMemoryInterval.start = alignedMemoryIntervalStart + size_ - 1; 
		...
		if(rLastMemoryBlock.freeSpace < size_)
		{
			...
			lastMemoryBlockId = this->memoryBlocks.add();	
			rLastMemoryBlock = this->memoryBlocks[lastMemoryBlockId];
			rLastMemoryBlock.create(this->memoryBlockSize, this->memReqs, this->memoryProperties, this->memoryVisability, this->upperBoundEntrySize, lastMemoryBlockId); 
