...
void Framebuffer::create(uint32_t width_, uint32_t height_, uint32_t layers_, RenderPass& rRenderpass_)
{
    ...
    VkResult result = vkCreateFramebuffer(Globals::renderer.mainDevice.logicalDevice, &frameBufferCreateInfo, nullptr, &this->vkHandle);
...
{
    vkDestroyFramebuffer(Globals::renderer.mainDevice.logicalDevice, this->vkHandle, nullptr);
